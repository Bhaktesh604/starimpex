...
const EmailBox = ({ exportRowData, handleOpenClose }: EmailBoxProps) => {
  ...
  return (
    <div className="max-w-[38.75rem] w-full bg-white rounded-xl mx-5 -mt-48">
      <div className="p-4 border-b-[1px] border-black/15">
        <h2 className="font-medium text-lg">Send Excel to Mail</h2>
      </div>
      <form className="p-4" onSubmit={sendStoneDetailsViaMail}>
        <p className="text-sm leading-5 font-normal pt-2 pb-4">
        ...
        <div className="flex justify-start items-center gap-x-2 mb-4">
          ...
          <input
            ...
            className="border border-primary/20 px-3 py-2 rounded-md placeholder:text-sm leading-5 placeholder:text-primary/60  placeholder:capitalize outline-none max-w-[25rem] w-full"
          />
        </div>
        <p className="text-red-600 ml-[1rem]">{emailError}</p>
        <div className="flex justify-end items-center gap-x-2 ">
          <button
            className="uppercase font-medium text-base leading-6 rounded-xl py-2 px-4 border border-black"
            onClick={handleEmailBox}
          >
            Cancel
          </button>
          <button
            className={`uppercase font-medium text-base leading-6 bg-tertiary text-white rounded-xl py-2 px-4 `}
            type="submit"
          >
            Send
          </button>
