...
export const getUploadSampleFile = async () => {
  const resp = await getAdminAxiosInstance().get(
    `/diamond/upload-file/download`,
    {
      responseType: "blob",
    }
  );
...
export const uploadDiamondsViaFile = async (file: any) => {
  const formData = new FormData();

  formData.append("file", file);

  const resp = await getAdminAxiosInstance().post(
    `/diamond/upload-via-file`,
    formData,
    {
      headers: {
